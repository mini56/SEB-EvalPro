...
const engineFile=path.join(root,'src','qwen-proposed-synthesis.js');

function fail(m){console.error('SEB EvalPro Qwen direct UI: '+m);process.exit(2)}
if(!fs.existsSync(htmlFile))fail('admin-bilan.html généré introuvable');
if(!fs.existsSync(engineFile))fail('moteur Qwen proposé introuvable');

let html=fs.readFileSync(htmlFile,'utf8').replace(/\r\n/g,'\n');
const engine=fs.readFileSync(engineFile,'utf8').replace(/\r\n/g,'\n');
if(html.includes('seb-qwen-proposed-direct-ui')){console.log('SEB EvalPro Qwen direct UI: déjà injecté.');process.exit(0)}
...
(()=>{
 'use strict';
 const KIND='seb-qwen-proposition-directe-v1';
 function candidate(){try{return JSON.parse(sessionStorage.getItem('candidat_data')||'{}')||{}}catch(_){return{}}}
 function rows(){
   const out={};
   for(const key of Object.keys(window.SebQwenProposed.ROWS||{})){
 ...
 window.sebQwenProposedPayload=()=>{
   const profile=window.SebQwenProposed.buildProfile({candidate:candidate(),rows:rows()});
   const payload={kind:KIND,profile};
   try{sessionStorage.setItem('seb_evalpro_qwen_proposed_profile_v1',JSON.stringify(payload))}catch(_){}
   window.sebQwenProposedLastProfile=profile;
...
console.log('SEB EvalPro: moteur JSON proposé par Qwen injecté dans le bilan courant.');
