...
function targets(r){return abandonMap[abandonKey(r)]||abandonMap[String(r?.page||'')]||[]}
...
function reasonSentence(r){const q=Array.isArray(r?.raisons)?r.raisons:[];if(q.includes('Je ne comprends pas la consigne'))return'Le motif signalé concerne la compréhension de la consigne.';if(q.includes('L’exercice est trop difficile'))return'Le niveau de difficulté de l’exercice a été signalé comme trop important.';if(q.includes('Fatigue, gêne ou douleur'))return'Une fatigue, une gêne ou une douleur a été signalée.';if(q.includes('Autre raison'))return'Un autre motif a été signalé et reste disponible pour l’administrateur.';return''}
function abandonLabel(r){return String(r?.exercice||r?.page||'Exercice').replace(/^Exercice\s+/i,'').trim()||'Exercice'}
function sentenceAbandons(out){abandons().forEach(r=>{out.push('L’exercice « '+abandonLabel(r)+' » a été abandonné.');const x=reasonSentence(r);if(x)out.push(x)})}
