...
function installUnifiedSynthesis(){
 ...
 engine.addEventListener('click',async()=>{
  ...
  try{
   ...
   setApplied(true,false,'Qwen rédige à partir du JSON proposé…');
   setStatus('Qwen rédige la synthèse à partir du JSON prétraité…');
   try{
    ...
    const qwenPayload=window.sebQwenProposedPayload?.();
    const answer=await window.sebEvalPro.rewriteSynthesisLocal(qwenPayload||deterministic);
    ...
    if(answer.fallback){
      ...
      const reason=String(answer.reason||'Reformulation refusée.').replace(/\s+/g,' ').trim();
      setApplied(true,false,'Refus IA : '+reason+' — '+passesLabel(answer.passes)+', '+elapsedLabel(answer.elapsedMs)+'.');
    }else{
 ...
 engine.addEventListener('click',async()=>{
  ...
  try{
   ...
   try{
    ...
    if(answer.fallback){const reason=String(answer.reason||'Reformulation refusée.').replace(/\s+/g,' ').trim();setStatus('Synthèse moteur conservée après contrôle SEB-IA.');setApplied(true,false,'Refus IA : '+reason+' — '+passes(answer.passes)+', '+elapsed(answer.elapsedMs)+'.')}
