...
function fail(message) {
  console.error('SEB EvalPro Build #134 - dictée dans le bilan : ' + message);
  process.exit(2);
}
...
// Le bilan est généré puis modifié par plusieurs correctifs historiques.
// On repère donc le bloc Expression écrite par ses données fonctionnelles plutôt
// que par une chaîne complète sensible aux petites variations de formatage.
const expressionNeedle = "Object.hasOwn(sc,'pageTexteTrous')";
const needle = html.indexOf(expressionNeedle);
if (needle < 0) fail('données Texte à trous du bloc Expression écrite introuvables');
if (html.indexOf(expressionNeedle, needle + expressionNeedle.length) >= 0) {
  fail('plusieurs blocs Expression écrite candidats détectés');
}
const start = html.lastIndexOf('const exp=', needle);
if (start < 0) fail('début du bloc Expression écrite introuvable');
const end = html.indexOf('let e=0,he=false;', needle);
if (end < 0 || end <= start) fail('fin du bloc Expression écrite introuvable');
...
for (const required of [
  "json('dictee_data',null)",
  "dictee?.status==='verified'",
  '+dictee.scoreSur20',
  'tt=15+pt+20+(dicteeOK?20:0)',
  'v=tr+p+g+d',
  "Dictée : '+d+' / 20"
]) {
  if (!html.includes(required)) fail('contrôle absent après correction : ' + required);
}

// Contrôles arithmétiques bloquants.
...
// Vérifier la syntaxe de tous les scripts de la page réellement produite.
...
console.log('SEB EvalPro Build #134 : dictée intégrée au bilan Expression écrite — 20 points ajoutés lorsque la dictée est validée; total 75 points avec les quatre composantes; seuils I/II/III inchangés.');
