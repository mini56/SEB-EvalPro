(function () {
  ...
  function responseInput(index) {
    return document.getElementById('reponse2_' + index);
  }

  function unitInput(index) {
    return document.getElementById('unite2_' + index);
  }
  ...
  function historicalState() {
    ...
    let found = false;
    for (let i = 1; i <= TOTAL; i += 1) {
      const answerKey = 'page2_q' + i;
      const unitKey = 'page2_unite' + i;
  ...
  function save() {
    ...
    for (let i = 1; i <= TOTAL; i += 1) {
      storedResponses['page2_q' + i] = result.details[i].reponse;
      storedScores['page2_q' + i] = result.details[i].correct ? 1 : 0;
      storedResponses['page2_unite' + i] = result.details[i].unite;
  ...
  function install() {
    ...
    document.getElementById('page2Pass')?.addEventListener('click', goNext);
    document.getElementById('page2Next')?.addEventListener('click', goNext);
